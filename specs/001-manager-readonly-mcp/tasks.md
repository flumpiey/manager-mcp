...
- [ ] T001 Create package layout `src/manager_mcp/__init__.py`, `src/manager_mcp/spec/`, and `tests/`
- [ ] T002 [P] Add `pyproject.toml` (hatchling, Python >=3.10, deps fastmcp>=2/httpx, optional `[dev]` pytest/pytest-asyncio/respx/ruff, console script `manager-mcp`)
- [ ] T003 [P] Add MIT `LICENSE`
- [ ] T004 [P] Add `.env.example` documenting `MANAGER_API_URL`, `MANAGER_API_KEY`, and write-flag hard-stop names (`MANAGER_MCP_ALLOW_WRITES`, `ALLOW_WRITES`, `MANAGER_MCP_WRITES`)
- [ ] T005 [P] Add `.gitignore` for Python/venv/`.env`/caches
- [ ] T006 [P] Add `src/manager_mcp/spec/README.md` explaining vendored OpenAPI is provenance-only (runtime reads live); leave placeholder for `api2.json` or empty note until vendored
...
- [ ] T007 Implement async GET-only httpx client in `src/manager_mcp/client.py` (`MANAGER_API_URL`/`MANAGER_API_KEY`, `X-API-KEY`, query allowlist `term|sortBy|sortByDesc|skip|pageSize|fields` + report date keys when configured; clear error if env missing; no post/put/patch/delete methods)
- [ ] T008 [P] Add `tests/test_client.py` with respx: missing config error, param cleaning (drops unknown keys), `X-API-KEY` header asserted, HTTP error propagation, empty-body handling

### Resource allowlist

- [ ] T009 Implement allowlist + `resolve()` in `src/manager_mcp/resources.py` (six collections + seven report shortcuts; collection form path `{path}-form/{key}`; paths from plan/research / OpenAPI)
- [ ] T010 [P] Add `tests/test_resources.py`: resolve hits for all allowlisted names, miss returns `None`, form-path helper for collections

### Write-guard + FastMCP shell

- [ ] T011 Implement write-flag hard-fail + lazy client + FastMCP app shell in `src/manager_mcp/server.py` (FR-001 MCP server via console script; write env `1|true|yes|on` or any other non-empty → raise per plan; import needs no config; entrypoint for `manager-mcp`)
- [ ] T012 [P] Add `tests/test_readonly_guard.py` (FR-004/FR-005): each write-related env name with truthy/non-empty → hard-fail; unset → guard allows init; registered tool names contain no create/update/delete verbs (even if only stub tools yet)
...
- [ ] T013 [P] [US1] Add failing/respx tests for `aged_receivables` tool in `tests/test_server_tools.py` (success path + auth error; period unsupported notice if no date params)

### Implementation

- [ ] T014 [US1] Register `aged_receivables` tool in `src/manager_mcp/server.py` (GET via client + resources resolve; period params only if descriptor `date_params` set)
- [ ] T015 [P] [US1] Ensure `customers` collection is resolvable for follow-up list/get (resources already; wire only if missing) in `src/manager_mcp/resources.py`
...
- [ ] T016 [P] [US2] Extend `tests/test_server_tools.py` for `aged_payables`, `bank_balances`, `trial_balance`, `profit_and_loss`, `balance_sheet`, `tax_summary` (respx; period disclosure when unsupported)

### Implementation

- [ ] T017 [US2] Register remaining report shortcut tools in `src/manager_mcp/server.py`
- [ ] T018 [P] [US2] Document dual bank exposure in tool descriptions (`bank_balances` vs `bank_accounts`) in `src/manager_mcp/server.py` / `resources.py`
...
- [ ] T019 [P] [US3] Add tests for `list_resources` in `tests/test_server_tools.py` (lists collections+reports; `read_only` true; boundary text present)

### Implementation

- [ ] T020 [US3] Implement `list_resources` in `src/manager_mcp/server.py` per `contracts/mcp-tools.md`
...
- [ ] T021 [P] [US4] Add respx tests for `list_records` in `tests/test_server_tools.py` (FR-011: assert `truncated` and/or `has_more`; term forwarded; unknown resource error)
- [ ] T022 [P] [US4] Add respx tests for `get_record` (`-form/{key}` path, 404 handling) in `tests/test_server_tools.py`

### Implementation

- [ ] T023 [US4] Implement `list_records` in `src/manager_mcp/server.py` (resource/term/sort/skip/page_size; FR-011 `truncated` and/or `has_more` in response)
- [ ] T024 [US4] Implement `get_record` in `src/manager_mcp/server.py` using collection `-form/{key}` sibling endpoint
...
- [ ] T025 [US5] Author `skills/manager-accounting/SKILL.md` (description triggers; read-only boundary; points at MCP tools / env config)
...
- [ ] T026 [P] Write `README.md` (config env vars, MCP client config JSON example, tool table including intentional bank dual path, roadmap noting opt-in writes are v2+ and currently hard-fail, multi-business caveat)
- [ ] T027 [P] Add `.github/workflows/ci.yml` matrix Python 3.10 and 3.12 running `ruff check` + `pytest`
- [ ] T028 [P] Vendor or stub `src/manager_mcp/spec/api2.json` provenance snapshot + confirm `spec/README.md` accuracy
- [ ] T029 Confirm console script `manager-mcp` launches FastMCP entry from `pyproject.toml` / `src/manager_mcp/server.py` (FR-001: MCP client can start the server)
- [ ] T030 Run full offline suite (`ruff check` + `pytest`) and fix gaps vs `quickstart.md`
